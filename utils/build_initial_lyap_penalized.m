function lf = build_initial_lyap_penalized(data, param)
...
sdpvar b_slack norm_slack                     % slack variable for objective

% Objective
obj = b_slack + param.kappa * norm_slack;
...
% We create the cost function kappa * norm(LYAP,2) + |b|
% This is handled using a second-order cone constraint
cnstr = [cnstr; 
    cone(LYAP, norm_slack); 
    b_slack + b >= 0; 
    b_slack - b >= 0];

% Penalty on lyapunov function coefficients
% vc_slack = sdpvar(nv, 1);
% obj = obj + param.eps(4)*sum(vc_slack);
...
lf.coeff = value(vc);
lf.basis = basis_v;
lf.others.lyap_err = abs(value(LYAP));
